...
class ImageVersionsWindow(Gtk.ApplicationWindow):
    ...
    def generate(self, thefile):
        ...
        p = subprocess.Popen(["git", "--no-pager","log","--pretty=%H	%ar",thefile], cwd=filepath, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        ...
        for i in lines:
            ...
            try:
                ...
                generateSVGprocess = subprocess.Popen(["git", "--no-pager","show",data[0]+":"+filename], cwd=filepath, stdout=f)
                f.close()
